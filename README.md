...
1. #### EC2 Setup:
   ...
   An **Amazon EC2 instance** is launched with **AWS Nitro Enclaves** and **TPM (Trusted Platform Module)** enabled. For this project, a **C7i.12xlarge** instance with a **150GB** volume is used to provide the necessary compute resources for running large language models. The Nitro Enclave partition isolates memory and CPU cores from the host instance, ensuring strong runtime separation.
...
2. **Create an Amazon S3 Bucket for model:**  
   Go to the AWS Console, open the Amazon S3 service, and create a new bucket. Give it a unique name (ex. `AWSACCOUNTNUMBER-medgemma-model`) and use the same region as your other project resources. Update the `S3_BUCKET_NAME` variable with your model bucket name in `envelope_encrypt_model.sh` and `run.sh`.
...
11. Clone this repository on the Amazon EC2 instance. Alternatively, you can transfer the repository via `rsync`
