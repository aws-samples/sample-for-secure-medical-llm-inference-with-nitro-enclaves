...
This project demonstrates how medical LLMs can be securely published and consumed using **AWS Nitro Enclaves**. It enables **model owners** to protect their intellectual property while allowing **consumers** to run private healthcare inferences on sensitive data within their own AWS account. This design establishes a **zero-trust boundary** between the model publisher and the healthcare organization consuming the model.
...
2. The **consumer** must guarantee that their sensitive prompts, PHI, and results never leave their AWS environment or get exposed to the model owner.
...
This solution leverages **AWS Nitro Enclaves**, which provide isolated, attested, and cryptographically verified environments for secure compute. By using Nitro Enclaves, both parties achieve their requirements:

- **Model confidentiality**:

  - The model is encrypted at rest with **AWS KMS**.
  - Only a Nitro Enclave that can provide a valid attestation document is authorized to decrypt the model.
  - This ensures the model can run, but cannot be extracted or reused outside the enclave.

- **Data confidentiality**:

  - The consumer’s sensitive input data and outputs are processed entirely inside the enclave within their AWS account.
  - The model owner has no access to the prompts, patient data, or results.
  - Data never leaves the enclave unless explicitly allowed by the consumer’s IAM policy.

- **Attestation and trust**:
  - Nitro Enclaves produce attestation reports signed by AWS hardware that prove the enclave is running the expected code.
  - Consumers can validate that no backdoors exist in the runtime environment before sending data to the model.

## Architecture Considerations

- The **enclave image** contains only the runtime environment and inference code—no model IP is embedded.
...
- Only essential interfaces are exposed:
  - **KMS** for secure key handling.
  - **S3** for encrypted model artifact storage.
...
For demonstration purposes, this solution is deployed in a **single AWS account**. Ideally, in production, it would be deployed **across separate AWS accounts**—one for the **model owner** and one for the **consumer**. Using **cross-account IAM permissions** and **encrypted model sharing via AWS KMS**, the model owner can securely provide access to their model without exposing the underlying weights or logic, while the consumer can run sensitive inferences within their own environment, maintaining strict data privacy and zero-trust principles.

### Steps

1. **EC2 Setup:**  
   An **Amazon EC2 instance** is launched with **Nitro Enclaves** and **TPM (Trusted Platform Module)** enabled. For this project, a **C7i.12xlarge** instance is used to provide the necessary compute resources for running large language models. The Nitro Enclave partition isolates memory and CPU cores from the host instance, ensuring strong runtime separation.

2. **Model Download:**  
   The **open-source MedGemma 4B model** is retrieved from [HuggingFace](https://huggingface.co/google/medgemma-4b-it) and stored in an **Amazon S3 bucket** within the consumer’s AWS account. This ensures that the model artifacts are both available for inference and fully controlled by the consumer.

3. **Encryption:**  
   The model is encrypted using **AWS KMS envelope encryption**. Only enclaves presenting a valid attestation document can request the decryption key from KMS, which prevents unauthorized access to the model weights outside the enclave.

4. **Nitro Enclave Build:**  
   A **Docker image** containing the **llama.cpp inference runtime** is built and deployed inside the enclave. The enclave contains no model weights by default—only the runtime environment and inference server—which ensures that the enclave image is transparent and inspectable by consumers prior to execution.

5. **Decryption and Model Loading:**  
   Once the Nitro Enclave is launched, it requests decryption of the model artifacts using its attestation credentials. The model is then securely decrypted **inside the enclave memory** and loaded by the **llama.cpp server**, meaning the decrypted model weights are never visible outside the enclave boundary.

6. **Medical Query:**  
   Users can submit either **text or image-based queries** to the model. Queries are sent via **vsock**, a secure communication channel from the client application to the model server inside the enclave. Image queries require users to upload images (such as radiology scans or pathology slides) to an **Amazon S3 bucket**. The upload event triggers an **Amazon SQS queue**, which signals the enclave to fetch and process the image using MedGemma’s multimodal capabilities. In both cases, responses are generated entirely within the enclave and securely returned to the user.

7. **Message History:**  
   Each interaction, including the user’s prompt and the model’s response, is logged into **Amazon DynamoDB**. This provides a persistent conversation history that enables traceability, follow-ups, and auditing while keeping PHI securely stored within the consumer’s account.

## Prerequisites

- AWS CLI configured with appropriate permissions
- AWS account with access to EC2 instances and Nitro Enclaves
...
1. **`create_ec2.sh`** - Creates EC2 instance with Nitro Enclaves enabled (C7i.12xlarge)
...
- **`client/client.sh`** - Configures services and launches the secure enclave
...
- **`server/run.sh`** - Main server script that runs MedGemma 4B inside the enclave
...
2. **Create an Amazon S3 Bucket for model:**  
   Go to the AWS Console, open the S3 service, and create a new bucket. Give it a unique name (ex. `AWSACCOUNTNUMBER-medgemma-model`) and use the same region as your other project resources. Update the `S3_BUCKET_NAME` variable in `envelope_encrypt_model.sh` and `run.sh`.

3. **Create an Amazon SQS queue:**  
   Go to the AWS Console, open the SQS service, and create a new queue. Select **Standard Queue**, provide a name, leave the rest as default, and click **Create queue**. Replace the `SQS_QUEUE_URL` variable in `image_processor.py` and `lambda_function.py` (in `assets` folder) with your URL.

4. **Create an Amazon DynamoDB table:**
   Go to the AWS Console, open the DynamoDB service, and create a new table. Make its partition key `ID` as a String type. Replace the `TABLE_NAME` variable with the table name and `REGION` variable with your region in `direct_query.py` and `image_processor.py` files.

5. **Create an AWS Lambda Function:**  
   Go to the AWS Console, open the Lambda service, and create a new function. Choose a name, select a runtime (e.g., Python 3.13), and paste in the Lambda function code from the `scripts`. Next, update the Lambda function's IAM role under Permissions in the Configuration tab with access to your Amazon S3 image bucket and the Amazon SQS queue you created with inline policy permissions. Finally, within the Lambda Designer, add a trigger, choose Amazon S3, and select your image bucket.
...
9. The script will create an EC2 called `MedGemmaNitroEnclaveDemo`. Once the EC2 is running, you will need to:

- **Add an IAM role**: Go to AWS IAM, create a role (i.e. `enclave-role`) and give it Amazon S3 access to the model bucket, Amazon SQS access to the queue you created, and Amazon DynamoDB access to the table using inline policy permissions. Afterwards, update your EC2 to use the role by going to the Security setting under the Actions dropdown, then modifying its IAM role.
  ...
  - SQS policy: Same as one in Step 5.

- **Increase the EBS storage**: Go to the Storage setting on the EC2 and modify the volume to be 150GB.
...
11. Clone this repository on the EC2 instance. Alternatively, you can transfer the repository via SCP

```
git clone <repository-url>
cd llm_enclave_medgemma

OR

scp -i /path/to/directory/llm_enclave_medgemma/nitro-enclave-key.pem -r /path/to/directory/llm_enclave_medgemma ec2-user@ec2-user@<PUBLIC_IP>:~
cd llm_enclave_medgemma
```
...
14. The various scripts will run to download the model, encrypt the model, build a Docker image to run llama.cpp, start an Enclave, and decrypt and run the model (this takes roughly ~10 minutes). The script runs in DEBUG MODE, so you will see various logs outputted. Wait until llama-server logs are outputted that the indicate the server is ready. You can then navigate to the `client` folder and run `direct_query.py` or `image_processor.py`. See below on how to run the Python scripts.

## Image-based Inference Examples (run from `client` folder):

1. Upload images from `assets` folder to your Amazon S3 image bucket.
